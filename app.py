...
def make_public_reading(reading):
    ...
    for field in reading:
        if field == 'id':
            new_reading['uri'] = url_for('get_reading',
reading_id=reading['id'], _external=True)
        else:
            new_reading[field] = reading[field]
    return new_reading

## Routes /weather/api/v1/readings GET ALL
@app.route('/weather/api/v1/readings', methods=['GET'])
@auth.login_required
def get_tasks():
    #   return jsonify({'readings': [make_public_reading(reading) for
    #   reading in readings]})
...
def create_reading():
    ...
    try:
       mariadb_connection = mariadb.connect(user='web', password='secretpasswo$
        cursor = mariadb_connection.cursor()
        cursor.execute("INSERT INTO reading (angle_x,angle_y,angle_z,acc_x,acc_$
...
def index():
   mariadb_connection = mariadb.connect(user='web', password='secretpassword', $
   cursor = mariadb_connection.cursor()
